if (window.PrimeFaces) {
	...
	PrimeFaces.converter = {
		...
		"javax.faces.DateTime" : {
			...
			convert : function(f, b) {
				...
				var e = PrimeFaces.util.ValidationContext, h = f
						.data("p-pattern"), g = f.data("p-dttype");
				var a = e.getLocaleSettings();
				try {
					return $.datepicker.parseDate(h, b, a)
				} catch (d) {
					var c = $.datepicker.formatDate(h, new Date(), a);
					if (g === "date") {
						throw e.getMessage(this.DATE_ID, b, c, e.getLabel(f))
					} else {
						if (g === "time") {
							...
							if (g === "both") {
								throw e.getMessage(this.DATETIME_ID, b, c, e
										.getLabel(f))
							}
						}
					}
	...
	PrimeFaces.validateInstant = function(f) {
		var e = PrimeFaces.util.ValidationContext, d = $(PrimeFaces
				.escapeClientId(f)), a = d.data(PrimeFaces.CLIENT_ID_DATA)
				|| d.attr("id"), c = d.data("uimessageid"), b = null;
		if (c) {
